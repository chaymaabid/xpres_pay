...
export default function Step1ConfirmOrder({ items, farmerName, onItemsChange, onNext }: Props) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-8 items-start">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    {item.stockAvailable !== undefined && (
                      <p className={`text-[11px] font-medium mt-0.5 ${
                        ...
                        {overStock
                          ...
                          : lowStock
                          ? `Low stock: ${item.stockAvailable} left`
                          : `In stock: ${item.stockAvailable}`}
                      </p>
                    )}
                  </div>
 
                  {/* Stepper — capped at stockAvailable */}
        ...
        <div className="px-5 py-4 bg-gray-50 border-t border-gray-100 space-y-2">
          ...
          <div className="flex justify-between text-sm text-gray-500">
            <span>Estimated Sales Tax (8.5%)</span><span>${tax.toFixed(2)}</span>
          </div>
